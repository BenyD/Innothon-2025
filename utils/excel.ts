...
const getEventTitles = (eventIds: string[]): string => {
  return eventIds
    .map(eventId => events.find(e => e.id === eventId)?.title || 'Unknown Event')
    .join(', ');
};
...
const formatGameDetails = (details: GameDetails): string => {
  ...
  const parts = [];
  if (details.game_type) parts.push(`Type: ${details.game_type}`);
  if (details.game_id) parts.push(`ID: ${details.game_id}`);
  if (details.team_tag) parts.push(`Tag: ${details.team_tag}`);
...
export const formatRegistrationForExcel = (registration: Registration) => {
  // Format the date
  const registrationDate = new Date(registration.created_at).toLocaleString('en-IN', {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
    hour: '2-digit',
  ...
  // Capitalize status
  const formattedStatus = registration.status.charAt(0).toUpperCase() + registration.status.slice(1);
  ...
  return {
    ...
    "Payment Date": registration.payment_date ? new Date(registration.payment_date).toLocaleString('en-IN') : 'Pending',
...
export const formatEventRegistrationForExcel = (registration: Registration, eventId: string) => {
  ...
  if (eventId === 'pixel-showdown' && registration.game_details) {
    return {
      ...eventSpecificFormat,
      "Game Type": registration.game_details.game_type || 'N/A',
      "Game ID": registration.game_details.game_id || 'N/A',
      "Team Tag": registration.game_details.team_tag || 'N/A',
    };
  }
